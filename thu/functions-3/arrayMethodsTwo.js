...
// CODE HERE
const lastI = cities.filter(x => x.endsWith('i'))
console.log(lastI)
...
// CODE HERE
const identifyFruit= fruits.forEach((x,i)=>{
    if (x.color==="red"){
...
// const totalPrice = foods.filter(function (el) {
//  return el.tags.includes('rice')
// }).map(x=>x.price).reduce((acc, curr)=> acc + curr)
