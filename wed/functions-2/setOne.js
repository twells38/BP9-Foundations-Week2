...
// CODE HERE
const printParameters = (part1, part2) => console.log(
    `The first parameter is ${part1}. The second parameter is ${part2}`
)
printParameters('parameter1', 'parameter2');
...
const callback = (fullName)=>`Hello my name is ${fullName}`
// INVOKE 'GREETING FUNCTION' HERE
greeting('Sornya','Wells',callback)
...
const pricesPlusTax = (prices, cb) => {
    for (let i = 0; i < prices.length; i++){
      //  let totalCost = []
        newCost = prices[i] + (prices[i] * .2)
        totalCost.push(newCost)
    }
    console.log(cb(totalCost))
}
...
// CODE HERE
const callback2 = totalCost =>`The new array plus tax: ${totalCost}`
pricesPlusTax(prices, callback2);
...
// CODE HERE
function multiplyingFactory(num1) { // higher order function
    return function (num2) {
        if (num1 >= 5) {
            console.log(num1 * num2) 
        } else {
            console.log(`Cant mu;tiple: the first number is smaller than 5`)
        }
    }
}
//const multiplyBy7 = multiplyingFactory(7) 
//multiplyBy7(5);
